...
- **Security:** HIPAA compliance, robust user authentication (Flask-Login), role-based access control, CSRF protection, comprehensive audit logging, secure deletion of original files, and PHI filtering at multiple layers. Account recovery, OAuth, EMR, document, and session security are hardened with rate limiting, secure tokens, isolation, and audit trails. Security alerting via Resend for account lockouts, brute force detection (10 attempts/5min threshold covering both existing and nonexistent usernames), and PHI filter failures.
