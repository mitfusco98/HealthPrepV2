...
Epic SMART on FHIR integration uses OAuth2 for authentication, token management, and session-based secure storage. It includes an enhanced `Patient` model, `FHIRDocument` model for DocumentReference management, and a dedicated FHIR service layer for bidirectional synchronization. **`services/comprehensive_emr_sync.py` is the sole authoritative EMR ingestion path** - it provides atomic upserts with SELECT FOR UPDATE row locking and PostgreSQL `on_conflict_do_update()` for duplicate key safety during document synchronization. Asynchronous processing via RQ (Redis Queue) handles batch prep sheet generation and background document processing. A PHI filter testing interface allows real-time detection and redaction testing. The system ensures Epic blueprint compliance through LOINC code mapping, consolidated data structures, and unit conversion. A comprehensive Epic Prep Sheet Write-Back System allows manual PDF generation with WeasyPrint (with base_url for CSS resolution), timestamped versioning, base64-encoded PDF attachments in FHIR R4 DocumentReference format, and automatic OAuth token refresh, including a living document concept. Organization-scoped `PrepSheetSettings` provide independent configuration per tenant. An Epic Dry-Run Mode enables safe testing without live data transmission. Epic FHIR client includes OperationOutcome parsing for detailed error messages, sandbox limitation detection, and structured error responses (`{'error': str, 'details': dict, 'is_sandbox_limitation': bool, 'status_code': int}`).
...
- **Security:** HIPAA compliance, robust user authentication (Flask-Login), role-based access control, CSRF protection, comprehensive audit logging, secure deletion of original files, and PHI filtering at multiple layers. Account recovery, OAuth, EMR, document, and session security are hardened with rate limiting, secure tokens, isolation, and audit trails. Security alerting via Resend for account lockouts, brute force detection (10 attempts/5min threshold covering both existing and nonexistent usernames), and PHI filter failures. Epic OAuth token refresh includes circuit breaker (max 2 retries) to prevent infinite retry loops when credentials are revoked. Enhanced PHI filter includes: financial IDs (account/routing/credit card numbers), government IDs (driver's license/passport/state ID), provider IDs (NPI/DEA/medical license with type-specific markers), PHI-bearing URLs, and context-aware patterns (Account #, Case #, Invoice #, etc. with digit requirement to avoid false positives).
- **Production Security Hardening (January 2026):**
  - **CORS:** Environment-based origin whitelist via `CORS_ALLOWED_ORIGINS` (blocks all cross-origin if not set in production)
  - **Encryption:** `ENCRYPTION_KEY` enforced in production with startup failure if missing
  - **Database:** SQLite blocked in production; PostgreSQL required
  - **Rate Limiting:** Redis-backed distributed rate limiting via `REDIS_URL` for multi-instance deployments
  - **CSP:** Configurable strict mode via `CSP_STRICT_MODE=true` (removes unsafe-inline/unsafe-eval)
  - **Security Audit:** `scripts/security_audit.py` for CI/CD integration with exit codes
  - **Documentation:** `docs/SECURITY_CHECKLIST.md` for pre-deployment verification
- **Extensibility:** Customizable screening logic, universal naming systems, variant management.
- **Deterministic Variant Selection:** During EMR sync, screening types are grouped by `base_name` (extracted via `_extract_base_name()` splitting on " - " delimiter). For each family, ONE variant is selected per patient using `_select_best_variant()` with deterministic sorting: `(-trigger_matches, -specificity_score, id)`. Scoring: trigger_matches counts patient conditions matching variant triggers; specificity_score assigns General (no triggers)=0, Condition-triggered=+10, Severity-specific=+5 per level (mild=5, moderate=10, severe=15, very_severe=20). Patients with trigger conditions (e.g., PCOS, severe COPD) receive ONLY the specialized variant protocol, not both general and variant screenings. Obsolete variant screenings are soft-archived with `status='superseded'` (preserving audit history) and filtered from UI via `Screening.status != 'superseded'` in all query paths (provider_scope, prep_sheet generator, API endpoints, main routes). Risk-stratified presets include severity levels in trigger conditions (e.g., "severe COPD", "moderate asthma") for proper variant matching.
...
- **Reliability:** Deterministic eligibility calculations, idempotent PHI redaction, match explanation audit trails, and atomic refresh operations using per-patient savepoints. Document processing events (document_processed, ocr_completed, phi_redacted, file_secure_deleted) logged with org/patient/user context via `utils/document_audit.py`.
...
- **Dual-Title Architecture:** FHIRDocument stores two title fields: `title` (LOINC-derived, PHI-free for UI display) and `search_title` (PHI-filtered via whitelist approach, preserves medical keywords for screening matching). This enables accurate keyword matching while maintaining HIPAA compliance.
- **AWS Backup/Recovery Architecture:** Designed for deterministic recovery with minimal data storage. Essential data (orgs, users, OCR transcripts, screening matches) backed up via RDS PITR and S3. Regenerable data (document metadata, patient demographics, screening due dates) reconstructed from Epic sync. Recovery preserves `/admin/documents` and `/screening/list` functionality. See `docs/SECURITY_WHITEPAPER.md` for full architecture.
- **Timezone-Aware Dormancy Rollover:** Organization model includes IANA timezone field (default 'UTC'), auto-detected from address during Epic OAuth. When appointment-based prioritization is enabled and non-priority processing is disabled, dormancy cutoff uses local midnight (via `utils/date_helpers.py`) for clean day-boundary transitions. Screenings processed before local midnight appear dormant immediately on page load without requiring EMR sync. Rolling 14-day window used for other configurations.
