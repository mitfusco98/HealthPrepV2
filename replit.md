...
- **Ralph Loop Self-Optimization:** 45 tests in `ralph_loop.py` ensure screening engine determinism, error-free operation, condition normalization, variant deduplication, and performance benchmarks. The loop validates: eligibility calculations (<3s for 100 calls), fuzzy detection (<15s for 10 calls), document matcher initialization (<2s), specific_screening_types filtering for targeted refresh, and completed screening preservation during variant archiving.
