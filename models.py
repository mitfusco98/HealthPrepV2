...
class Patient(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    fhir_id = db.Column(db.String(100), unique=True, nullable=True)
...
class ScreeningType(db.Model):
    ...
    keywords = db.Column(Text, nullable=False)  # JSON array of keywords
    ...
    def get_trigger_conditions(self):
        ...
        return self.trigger_conditions if self.trigger_conditions else []

    def set_trigger_conditions(self, conditions_list):
        """Set trigger conditions from a list"""
        self.trigger_conditions = conditions_list
...
class Screening(db.Model):
    ...
    matched_documents = db.Column(Text)  # JSON array of document references
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class AdminLog(db.Model):
    ...
    details = db.Column(Text)  # JSON details
