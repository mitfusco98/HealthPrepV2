...
class ComprehensiveEMRSync:
    ...
    def _sync_patient_documents(self, patient: Patient, last_encounter_date: Optional[datetime],
        ...
        try:
            ...
            for fhir_document in documents_data.get('entry', []):
                ...
                # Skip HealthPrep-generated documents to prevent circular reingestion
                # This catches prep sheets written back to Epic via write_prep_sheet_to_epic
                if self._is_healthprep_generated_document(document_resource):
                    logger.info(f"Skipping HealthPrep-generated document: ID={doc_id}")
                    documents_skipped += 1
                    continue
    ...
    def _is_healthprep_generated_document(self, document_resource: Dict) -> bool:
        """
        Detect if a FHIR DocumentReference was generated by HealthPrep.
        
        This prevents reingestion of prep sheets written back to Epic, which would
        cause circular data flow and potentially malformed OCR content.
        
        Detection methods (aligned with FHIRDocument._check_healthprep_identifier):
        ...
        Returns:
            True if document was generated by HealthPrep, False otherwise
        """
        try:
            # Method 1 & 2: Check for HealthPrep identifiers (current and legacy)
            identifiers = document_resource.get('identifier', [])
            for identifier in identifiers:
                system = identifier.get('system', '')
                value = identifier.get('value', '')
                
                # Current standard identifier
                if system == 'urn:healthprep:document':
                    logger.debug(f"Skipping HealthPrep-generated document (current identifier match)")
                    return True
                
                # Legacy identifier system
                if system == 'https://healthprep.io/fhir/identifier' and value == 'healthprep-generated':
                    logger.debug(f"Skipping HealthPrep-generated document (legacy identifier match)")
                    return True
            
            # Method 3: Check category coding for HealthPrep marker
            categories = document_resource.get('category', [])
            for category in categories:
                codings = category.get('coding', [])
                for coding in codings:
                    if coding.get('code') == 'healthprep-prep-sheet':
                        logger.debug(f"Skipping HealthPrep-generated document (category match)")
                        return True
            
            # Method 4: Check author display for HealthPrep System
            authors = document_resource.get('author', [])
            ...
            logger.warning(f"Error checking HealthPrep document origin: {str(e)}")
            return False
